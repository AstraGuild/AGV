...
contract SeedAGV {
            ...
            if(isVestingTime == true){
                if(investors[_msgSender()].claimCounter < vestingDuration){
                    if(investors[_msgSender()].previousClaimTime != 0){
                        ...
                        // if(block.timestamp > investors[_msgSender()].previousClaimTime + 3*60) return true; // testing purpose
                        else return false;
                    } else {
                        return true;
                    }
    ...
    function generateToken() external isInvestorExist {
        ...
        require(investors[_address].isTokenGenerated == false,"Token aleready generated.");
